...
function Header(){
    ...
    let [precio, setPrecio] = useState(0.00)
    let _precio = 0
    ...
    const clienteLoggedString = localStorage.getItem('clienteLogged')

    if(!clienteLogged) clienteLogged = clienteLoggedString ? JSON.parse(clienteLoggedString) : null
                    ...
                    <div className="row m-0 p-0" style={{backgroundColor:"#2c2c2c"}}>
                        ...
                        <div className="col-3">
                            <div className="btn-group btn-sm">
                                <a href="#" className="btn btn-dark btn-sm"><img src="/images/botonMINIcontacto.png" /></a>
                                { /* boton del login, siempre y cuando  no exista estado de sesion...
                                   ...
                                   clienteLogged || localStorage.getItem(clienteLogged)? 
                                   (
                                    <Link to="/Cliente/Panel/MiPanel" className="btn btn-dark btn-sm">{ clienteLogged.datoscliente.cuenta.email}</Link>
                                   )
                                   :
                                   (
                                    
                                    <Link to="/Cliente/Login" className="btn btn-dark btn-sm"><img src="/images/botonMINIlogin.png" /></Link>
                                   )
                                }
                            </div>
                        </div>
                            ...
                            <div className="col-4 p-0">
                                <div className="w-50 h-100">
                                    ...
                                    <Link to="/Pedido/MostrarPedido"  className="h-100 bg-white border border-dark border-1 btn btn-light">
                                        ...
                                        <span className="text-dark badge badge-danger  ps-1 pe-1"> <small>Total:</small> <span className="text-end" id="precioCarrito" style={{width: "110px", fontSize: "1.2em"}}>{precio} €</span></span>
                                        {/*add-pedido*/}
